...
| Map                    | Google Maps 3D (@vis.gl/react-google-maps, @googlemaps/markerclusterer) with dark theme, tilt, AdvancedMarkerElement |
...
│       │   ├── /map          # MapView (Google Maps 3D), AssetMarker, MissionZone, DrawingTools, AssetTrail
...
│       ├── /context          # AuthContext (React 19 use() compatible)
│       ├── /hooks            # useMissions, useAssets, useEvents, useChat, usePermissions (React Query + Socket.IO)
...
- Google Maps 3D with dark theme, tilted view, AdvancedMarkerElement custom markers
- Marker clustering (MarkerClusterer) when zoomed out
- Map drawing tools for creating mission area polygons
- Animated polylines showing asset movement paths (trail history)
- Dashboard with stat cards (extracted StatCard component) and Recharts charts (extracted Charts components)
- Activity feed (extracted ActivityFeed component)
- Mission management (CRUD + status workflow + timeline updates) — extracted MissionCard, MissionDetail, MissionForm
- Asset management (CRUD + type filters) — extracted AssetCard, AssetDetail, AssetForm
- Events/activity log page
- Chat page with channels
- Chat slide-out panel (ChatPanel component)
- Framer Motion animations
- Dark military theme with glassmorphism
...
- Role-based UI restrictions (usePermissions hook, Sidebar nav filtering, conditional CRUD buttons)
- Loading skeletons (SkeletonCard, SkeletonRow, SkeletonText, SkeletonChart, SkeletonMap with shimmer animation)
- Connection status indicator (TopBar + Sidebar socket connected/disconnected)
- React 19 use() hook for reading AuthContext
- AuthContext provided in App.jsx, consumed with use() in usePermissions
- Modular component architecture (components/map, components/dashboard, components/missions, components/assets)
